...
// 	const [sortingOption, setSortingOption] = useState<string>('productPrice');
...
// 	const filteredProducts = searchOrderHandlear;
...
const ShopTopSortingSection = () => {
	...
	const handleSortingChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		if (selectedOption === 'lowToHigh') {
			setProductSearch({ ...productSearch, page: 1, order: 'productPrice' });
		} else if (selectedOption === 'highToLow') {
			setProductSearch({ ...productSearch, page: 1, order: '-productPrice' }); // teskari tartibda narx
		} else if (selectedOption === 'productViews') {
			setProductSearch({
				...productSearch,
				page: 1,
			});
		} else {
			setProductSearch({ ...productSearch, page: 1, order: 'createdAt' }); // default
		}
	};
			...
			<div className="row align-items-center">
				<div className="col-md-8 text-center text-md-start" data-aos="fade-up" data-aos-duration="500">
					<span className="shop-notice-result">
						{allMenu.length === 0
							? 'No Products to Show'
							: `Showing ${startIndex + 1}-${endIndex} of ${allMenu.length} Products`}
					</span>
				</div>
				<div className="col-md-4 text-center text-md-end" data-aos="fade-up" data-aos-duration="500">
					<div className="shop-notice-select">
						<Form.Select value={sortingOption} onChange={handleSortingChange}>
							...
							<option value="sortAToZ">Most Viewed</option>
						</Form.Select>
					</div>
				</div>
			</div>
